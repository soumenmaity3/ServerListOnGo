...
public interface ProductModelRepository extends JpaRepository<ProductModel,Long>, JpaSpecificationExecutor<ProductModel> {
    ...
    @Query(value = "SELECT * FROM listongo_product WHERE who_admin=:userId", nativeQuery = true)
    List<ProductModel> findAllForAdminSpecific(Long userId);

    @Transactional
    @Modifying
    @Query(value = "UPDATE listongo_product SET who_admin=:adminId WHERE id=:imaId",nativeQuery = true)
    int makeApproveAdmin(Long imaId,Long adminId);
